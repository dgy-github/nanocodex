...
- 扩大 context regression tests，覆盖大工具输出、长会话、memory recall 竞争预算和长上下文质量验证；再评估是否加入模型引导的 focus compaction。

4. Tool search

当前差距：约 20-30% 仍未补齐。

已有 schema gating、MCP namespace-aware 排序、27 条覆盖 core tools/MCP connectors/release packaging 的 gold-case 回归、每轮 visible tools / called tools trace、`/tools eval [N]` 离线 schema-recall 报告，以及确定性 MCP category/capability hints。下一层缺口是扩大真实 trace 规模，并把 missed samples 继续沉淀成更丰富的类别体系。

下一步：

- 基于 `/tools eval [N]` 的 missed samples 扩展 MCP tool category ontology。
- 积累更大的真实 trace corpus，把 eval 接入 release/benchmark 报告。
...
- 已增强 tool_search：评分会分解 MCP server/tool namespace，MCP 工具 description 带 server/tool 元数据和确定性 category/capability hints，并新增 27 条跨类别 tool-selection gold cases，覆盖 core tools、MCP connectors 和 release packaging。
- 已新增 `TaskLedger`：CLI 与 Tauri 每轮完成后写 `.nanocodex/task-ledger.jsonl`，CLI 支持 `--budget-report` 和 `/budget report`，Tauri Usage 面板可读取最近任务报告；报告已加入平均耗时、预算耗尽率、模型/工具预算利用率，以及 visible tools / called tools trace；CLI `/tools eval [N]` 可把 trace 汇总成 schema recall / missed calls / MCP recall 报告。
...
5. 扩大 context regression tests、长上下文质量验证和模型引导 focus compaction，进一步缩小 context editing 差距。
