...
`nanocodex` 有 MCP stdio 客户端、工具注册表、审批和 GUI 连接状态可见性，但还不是“平台 connector”。缺少 OAuth/remote auth UX、可安装 connector registry、工具权限审计和大规模工具排序。
...
- 增加 connector auth/permission 治理和工具权限审计。
