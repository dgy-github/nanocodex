...
| Semantic memory | `.ncx/memory/LEARNINGS.md`、query-scoped recall、`remember`、启发式/LLM merge、`.ncx/memory/PROPOSALS.md` review queue、CLI `/memory accept/reject` 和 Tauri Memory 面板接受/拒绝已有。 | 自动从纠错/复盘提炼 memory proposals、embedding/vector 可选 provider、大批量 review 的 edit queue。 |
...
当前 memory 是本地 project memory，适合仓库级经验；已具备可审计的 verified/pending 两层文件和 CLI/Tauri review queue，但还不是会自动从纠错、复盘、release checklist 中稳定提炼建议的长期 memory 系统。
...
- 从失败修复、用户纠正、release checklist 中自动生成 memory proposals。
...
- 已新增 memory proposal review queue：候选经验写入 `.ncx/memory/PROPOSALS.md`，CLI `/memory` 可 propose/accept/reject，Tauri Memory 面板可接受/拒绝，只有接受后的内容才进入 `.ncx/memory/LEARNINGS.md` 并参与 recall。
