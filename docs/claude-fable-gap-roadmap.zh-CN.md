...
已有 schema gating、MCP namespace-aware 排序和 24 条覆盖 core tools、MCP connectors、release packaging 的 gold-case 回归。下一层缺口不再只是固定样例数量，而是真实任务 trace：需要确认 tool search 在减少干扰的同时不会错过必要工具。

下一步：

- 记录每轮 visible tools 与实际 called tools，并用真实 trace 反推排名回归。
