...
| Tool search / connectors | 66-76% | 工具 catalog、namespace-aware `tool_search`、GUI MCP runtime 状态、24 条跨类别 gold-case 排名测试、visible-vs-called task-ledger trace，以及可审计的 `connectors.toml` install spec 已降低 schema 和 connector 歧义；还缺远程 auth/OAuth UX、托管 registry、基于真实 trace 的排名评测和大规模动态工具排序。 |
