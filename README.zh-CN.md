...
| Semantic memory | 62-72% | query-scoped lexical-semantic recall、`remember`、LLM merge、CLI/Tauri memory proposal review queue，以及 handoff/release 文档提炼 proposal 已有；还缺从实时纠错/失败修复自动生成 proposal、更强 embedding/vector 检索，以及大批量 review 的编辑流。 |
...
- **记忆提议**（`.ncx/memory/PROPOSALS.md`）—— 有用但尚未可信的候选经验。`remember`
  ...
  `/memory reject <id>`，`/memory harvest [path]` 可从 handoff/release 文档提炼候选；
  Tauri Memory 面板也能从文档提炼并提供接受/拒绝按钮。提议被接受前不会进入 `LEARNINGS.md`，
  也不会被召回。
...
- Memory 面板可查看项目笔记、新增 verified note、从 handoff/release 文档提炼待审提议、
  review 待审记忆提议并接受/拒绝、打开 `LEARNINGS.md`、启发式去重和 LLM 记忆合并。
