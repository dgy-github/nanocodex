...
- **Semantic memory：** 每轮都会按当前 prompt 做 query-scoped 记忆召回，并以发送时
  ...
  现在可声明 OpenAI-compatible embedding provider/model/base URL/API-key env var；当环境变量存在时，
  runtime recall 会用 `.ncx/memory/EMBEDDINGS.json` 缓存外部 embedding 来增强排序，网络、配置或 key
  不可用时自动回退到本地确定性 vector sidecar。
...
截至 2026-06-30，公开 Claude Code 文档描述的是比本地开源 harness 更大的 Anthropic
...
| Semantic memory | 80-86% | query-scoped lexical-semantic recall、本地 vector sidecar recall、可选 OpenAI-compatible 外部 embedding recall 及可审计的 `.ncx/memory/EMBEDDINGS.json` sidecar、`remember`、LLM merge、CLI/Tauri proposal review、提议编辑、批量接受/拒绝、handoff/release 文档提炼、运行时纠正/失败 proposal 提炼都已具备；还缺平台级长期 memory、更广 provider 支持和更大规模 recall 质量评估。 |
...
- **项目记忆**（`.ncx/memory/LEARNINGS.md`）—— 经过验证的项目笔记，每轮按当前 prompt
  ...
  `/memory index` 或 Tauri Memory 面板重建。配置 `memory_embedding_provider = "openai-compatible"`
  或 `"openai"`，并填好 model/base URL/API-key env var 后，runtime recall 还会把 OpenAI-compatible
  embedding 缓存在 `.ncx/memory/EMBEDDINGS.json` 里增强排序。`/memory` 会显示
  `embedding_backend`、`embedding_provider` 和 `embedding_gap`；GUI Settings 可保存
  `memory_embedding_provider`、`memory_embedding_model`、`memory_embedding_base_url`、
  `memory_embedding_api_key_env`，但密钥本身仍只放在环境变量里。
